...
public class NetTaskManager {
    ...
    public NetTaskManager(TaskManager taskManager, GameClient client) {
        this.taskManager = Objects.requireNonNull(taskManager, "TaskManager不能为null");
        this.client = client;
    }
    ...
    public void completeOneStep(String taskName) throws IOException {
        ...
        // 本地更新
        task.completeOneStep();

        Map<String, String> payload = new HashMap<>();
        payload.put("taskName", taskName);
        payload.put("completedSteps", String.valueOf(task.getCompletedSteps()));

        try {
            System.out.println("[CLIENT-SEND] TASK_UPDATE -> "
                    + taskName + " steps=" + task.getCompletedSteps());
            if (client != null) {
