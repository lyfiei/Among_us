...
public class Main extends Application {
    ...
    private static Scene mainScene; // 主 Scene，切换时只替换 root
    ...
    public void start(Stage stage) throws Exception {
        ...
        mainScene = new Scene(splashRoot, 1000, 600); // 用 mainScene 保存
        ...
        fadeOut.setOnFinished(event -> loadStartMenu());
    ...
    private void loadStartMenu() {
        try {
            ...
            StackPane startRoot = loader.load();

            controller = loader.getController(); // 获取控制器

            // 如果 mainScene 还没创建，用 startRoot 初始化
            if (mainScene == null) {
                mainScene = new Scene(startRoot, 1000, 600);
            } else {
                mainScene.setRoot(startRoot); // 替换 root
            }
    ...
    public static void joinGame() {
        ...
        // 匹配界面
        MatchUI matchUI = new MatchUI(5);
        GameApp.setMatchUpdateListener((current, total) -> matchUI.onMatchUpdate(current, total));

        // 安全切换 root
        mainScene.setRoot(matchUI);
    }
...
//        return primaryStage;
//    }
//    private static Main instance;  // 保存 Application 实例
//
//    public Main() {
//        instance = this; // 构造时赋值
//    }
...
//        game = new GameApp(new Pane());
//
//        game.setStartMenuController(startMenuController);
//        Pane root = new Pane();
//        // 8. 创建 Scene 并显示
